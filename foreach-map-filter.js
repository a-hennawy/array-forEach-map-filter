...
function onlyEvenValues(arr) {
  const evenNumbers = arr.filter(function (val) {
    if (val % 2 === 0) {
      return true;
    }
  });
...
function addKeyAndValue(arr, key, value) {
  const arrayUpdate = arr.forEach(function (val) {
    val[key] = value;
  });
...
returns an object with the keys as the vowel and the values as the number of times the vowel appears in the string. This function should be case insensitive so a lowercase letter and uppercase letter should count
...
function doubleValuesWithMap(arr) {
  const mappedArray = arr.map(function (val) {
    return val * 2;
  });
  return mappedArray;
}
...
function filterByValue(arr, key) {
  const filteredValue = arr.filter(function (obj) {
...
a value and returns the first element in the array that has the same value as the second parameter or
